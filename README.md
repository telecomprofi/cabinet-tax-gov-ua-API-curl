...
```
  ...
  ### 1. check if that is valid Auth string by 
```echo <MIINE kjhdkjbjhho xjbsjb> | base64 -d 
```
  output of base64 should contain some Ukrainian text about tax id owner (since it is concatenation of your TaxID signed by your digital Ceritficate and then encoded in base64
  
 ### 2. curl the following URL and paste authorization string into -d Authorization " " header
```
  curl https://cabinet.tax.gov.u a-H 'Content-Type: application/json' \
